<script lang="ts">
  ...
  async function removeWishlist(id: number) {
    if (confirm('Hapus wishlist ini?')) { await deleteWishlist(id); await loadData(); }
  }
  ...
  async function removeUkuran(id: number) {
    if (confirm('Hapus ukuran ini?')) { await deleteUkuran(id); await loadData(); }
  }
</script>

<div class="space-y-6">
  <h1 class="text-2xl font-bold">🎁 Wishlist & Ukuran</h1>

  <div class="tabs tabs-boxed">
    <button class="tab" class:tab-active={tab === 'wishlist'} onclick={() => tab = 'wishlist'}>🎁 Wishlist Hadiah</button>
    <button class="tab" class:tab-active={tab === 'ukuran'} onclick={() => tab = 'ukuran'}>📏 Ukuran Pasangan</button>
  </div>
  ...
  {#if tab === 'wishlist'}
    <div class="flex justify-end">
      <button onclick={() => { editWishlistId = null; formWishlist = { nama_barang: '', harga_estimasi: 0, status: 'belum_dibeli', occasion: 'random', notes: '' }; showFormWishlist = true; }}
              class="btn btn-primary btn-sm"><Plus class="w-4 h-4" /> Tambah</button>
    </div>

    {#if showFormWishlist}
      <div class="card bg-base-100 shadow">
        <div class="card-body">
          <h3 class="card-title">{editWishlistId ? 'Edit' : 'Tambah'} Wishlist</h3>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div class="form-control">
              <label class="label"><span class="label-text">Nama Barang</span></label>
              <input type="text" class="input input-bordered" bind:value={formWishlist.nama_barang} placeholder="Nama barang" />
            </div>
            <div class="form-control">
              <label class="label"><span class="label-text">Estimasi Harga</span></label>
              <input type="number" class="input input-bordered" bind:value={formWishlist.harga_estimasi} />
            </div>
            <div class="form-control">
              <label class="label"><span class="label-text">Occasion</span></label>
              <select class="select select-bordered" bind:value={formWishlist.occasion}>
                {#each OCCASIONS as o}<option value={o}>{o}</option>{/each}
              </select>
            </div>
            <div class="form-control">
              <label class="label"><span class="label-text">Status</span></label>
              <select class="select select-bordered" bind:value={formWishlist.status}>
                {#each STATUS_OPTIONS as s}<option value={s}>{s.replace('_', ' ')}</option>{/each}
              </select>
            </div>
          </div>
          <div class="form-control">
          ...
          <div class="flex gap-2 mt-3">
            <button onclick={saveWishlist} class="btn btn-primary">Simpan</button>
            <button onclick={() => showFormWishlist = false} class="btn btn-ghost">Batal</button>
          </div>
        </div>
      </div>
    {/if}
    ...
    {:else if wishlists.length === 0}
      <div class="text-center py-12 text-base-content/50"><p class="text-4xl mb-4">🎁</p><p>Belum ada wishlist.</p></div>
    {:else}
      <div class="space-y-2">
                ...
                <div class="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div class="flex gap-2 mt-1">
                      <span class="badge badge-xs">{item.occasion}</span>
                      <span class="badge badge-xs" class:badge-success={item.status === 'sudah_dibeli'} class:badge-ghost={item.status === 'belum_dibeli'}>{item.status.replace('_', ' ')}</span>
                    </div>
                  </div>
                </div>
                <div class="flex items-center gap-3">
  ...
  {:else}
    <div class="flex justify-end">
      <button onclick={() => showFormUkuran = true} class="btn btn-primary btn-sm"><Plus class="w-4 h-4" /> Tambah Ukuran</button>
    </div>
    ...
    {#if showFormUkuran}
      <div class="card bg-base-100 shadow">
        <div class="card-body">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div class="form-control">
              ...
              <select class="select select-bordered" bind:value={formUkuran.kategori}>
                {#each UKURAN_KATEGORI as k}<option value={k}>{k}</option>{/each}
              </select>
            </div>
            <div class="form-control">
              <label class="label"><span class="label-text">Ukuran</span></label>
              <input type="text" class="input input-bordered" bind:value={formUkuran.ukuran} placeholder="M, 38, 42, dll" />
            </div>
          </div>
          <div class="form-control">
            <label class="label"><span class="label-text">Brand Favorit</span></label>
          ...
          <div class="flex gap-2 mt-3">
            <button onclick={saveUkuran} class="btn btn-primary" disabled={!formUkuran.ukuran}>Simpan</button>
            <button onclick={() => showFormUkuran = false} class="btn btn-ghost">Batal</button>
          </div>
        </div>
      </div>
    {/if}
    ...
    {#if ukurans.length === 0}
      <div class="text-center py-12 text-base-content/50"><p class="text-4xl mb-4">📏</p><p>Belum ada data ukuran.</p></div>
    {:else}
          ...
          <div class="card bg-base-100 shadow">
            <div class="card-body">
              <div class="flex justify-between items-start">
                <h3 class="font-bold">{item.kategori}</h3>
              ...
              {#if item.brand_favorit}
                <p class="text-xs text-base-content/50">Brand: {item.brand_favorit}</p>
              {/if}
              {#if item.notes}
