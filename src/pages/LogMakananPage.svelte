...
<div class="space-y-6">
  <div class="flex items-center justify-between">
    <h1 class="text-2xl font-bold">📋 Log Makanan</h1>
    <button onclick={() => showForm = !showForm} class="btn btn-primary btn-sm"><Plus class="w-4 h-4" /> Catat</button>
  </div>
  ...
  {#if showForm}
    <div class="card bg-base-100 shadow">
      <div class="card-body">
        <h3 class="card-title text-lg">Catat Makanan</h3>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div class="form-control">
            <label class="label"><span class="label-text">Waktu</span></label>
          ...
          <div class="form-control">
            <label class="label"><span class="label-text">Makanan</span></label>
            <input type="text" class="input input-bordered" bind:value={form.makanan} list="makanan-datalist" placeholder="Nama makanan" />
          ...
          <div class="form-control">
            <label class="label"><span class="label-text">Mood Sebelum</span></label>
            <select class="select select-bordered" bind:value={form.mood_sebelum}>
              {#each MOOD_OPTIONS as m}
                <option value={m}>{m ? MOOD_EMOJI[m] + ' ' + m : '--'}</option>
              {/each}
            </select>
          </div>
          <div class="form-control">
            <label class="label"><span class="label-text">Mood Sesudah</span></label>
        ...
        <div class="flex gap-2 mt-3">
          <button onclick={addLog} class="btn btn-primary" disabled={!form.makanan || !form.waktu}>Simpan</button>
  ...
  {:else if items.length === 0}
    <div class="text-center py-12 text-base-content/50">
      ...
      <p>Belum ada log makanan untuk tanggal ini.</p>
    </div>
  {:else}
    <div class="space-y-2">
      {#each items as item}
