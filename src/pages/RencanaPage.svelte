<script lang="ts">
  ...
  const STATUS_LABELS: Record<string, string> = $derived({
    wishlist: tr().rencana.statuses.wishlist,
    rencana: tr().rencana.statuses.rencana,
    booked: tr().rencana.statuses.booking,
    visited: tr().rencana.statuses.dikunjungi,
  });
  ...
  async function remove(id: number) {
    if (confirm(tr().rencana.deleteConfirm)) { await deleteRencana(id); await loadData(); }
  }
</script>

<div class="space-y-6">
  <div class="flex items-center justify-between">
    <h1 class="text-2xl font-bold">{tr().rencana.heading}</h1>
    <button onclick={openAdd} class="btn btn-primary btn-sm"><Plus class="w-4 h-4" /> {tr().rencana.add}</button>
  </div>

  {#if showForm}
    <div class="card bg-base-100 shadow">
      <div class="card-body">
        <h3 class="card-title">{editId ? tr().rencana.editTitle : tr().rencana.addTitle}</h3>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div class="form-control">
            <label class="label"><span class="label-text">{tr().rencana.name}</span></label>
            <input type="text" class="input input-bordered" bind:value={form.nama} placeholder={tr().rencana.namePlaceholder} />
          </div>
          <div class="form-control">
            <label class="label"><span class="label-text">{tr().rencana.category}</span></label>
            <select class="select select-bordered" bind:value={form.kategori}>
              {#each KATEGORI as k}<option value={k}>{tr().rencana.categories[k] || k}</option>{/each}
            </select>
          </div>
          <div class="form-control">
            <label class="label"><span class="label-text">{tr().rencana.location}</span></label>
            <input type="text" class="input input-bordered" bind:value={form.lokasi} placeholder={tr().rencana.locationPlaceholder} />
          </div>
          <div class="form-control">
            <label class="label"><span class="label-text">{tr().rencana.status}</span></label>
          ...
          <div class="form-control md:col-span-2">
            <label class="label"><span class="label-text">{tr().rencana.mapsUrl}</span></label>
            <input type="url" class="input input-bordered" bind:value={form.maps_url} placeholder={tr().rencana.mapsPlaceholder} />
            <span class="text-xs text-base-content/50 mt-1">{tr().rencana.mapsHint}</span>
          </div>
          <div class="form-control">
            <label class="label"><span class="label-text">{tr().rencana.estimatedCost}</span></label>
        ...
        <div class="card bg-base-100 shadow">
          <div class="card-body">
            ...
            <div class="space-y-1 mt-2">
              <span class="badge badge-sm badge-outline">{tr().rencana.categories[item.kategori] || item.kategori}</span>
              {#if item.lokasi}
            ...
            <div class="flex gap-1 justify-between items-center mt-2">
              <div class="flex gap-1">
                <a href={getGoogleMapsUrl(item)} target="_blank" rel="noopener" class="btn btn-xs btn-ghost" title={tr().rencana.openMaps}>
