...
{:else if error}
  <div class="alert alert-error">
    <p class="font-medium">{tr().dashboard.loadError}</p>
    <p class="text-sm font-mono mt-2">{error}</p>
    <button onclick={() => { error = ''; loaded = false; loadDashboard(); }} class="btn btn-sm btn-ghost mt-2">{tr().dashboard.retryLoad}</button>
  </div>
{:else}
  <div class="space-y-6">
    <div class="flex items-center gap-3">
      <Heart class="w-8 h-8 text-error" />
      <h1 class="text-2xl font-bold">{tr().dashboard.heading}</h1>
    </div>
    ...
    <div class="card bg-base-100 shadow">
      <div class="card-body">
        <h2 class="card-title flex items-center gap-2"><Bell class="w-5 h-5" /> {tr().dashboard.reminders}</h2>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div class="card bg-base-100 shadow lg:col-span-2">
        <div class="card-body">
          <h2 class="card-title">💡 {tr().dashboard.recommendations}</h2>
          {#if rekomendasi}
            ...
            {#if rekomendasi.makanan.length > 0}
              <div class="mt-3">
                <p class="font-medium mb-2">{tr().dashboard.foodRec}</p>
            ...
            {#if rekomendasi.warnings.length > 0}
              <div class="mt-3">
                <p class="font-medium text-error mb-2">{tr().dashboard.avoid}</p>
                <div class="space-y-1">
                  {#each rekomendasi.warnings.slice(0, 3) as w}
                    <p class="text-sm text-error">
                      ❌ "{w.kata_kalimat_jangan}" → ✅ "{w.alternatif_aman || tr().dashboard.sayNothing}"
                    </p>
                  {/each}
                </div>
              </div>
        ...
        <div class="card bg-base-100 shadow">
          <div class="card-body">
            ...
            {#if moodToday}
              ...
              {/if}
            {:else}
              <p class="text-base-content/50 text-center">{tr().dashboard.noMood}</p>
            {/if}
            <button onclick={() => setRoute('mood')} class="btn btn-sm btn-ghost mt-2">{tr().dashboard.logMood}</button>
          </div>
        </div>

        <div class="card bg-base-100 shadow">
          <div class="card-body">
            <h2 class="card-title">📅 {tr().dashboard.upcoming}</h2>
            ...
            {:else}
              <p class="text-base-content/50 text-sm">{tr().dashboard.noUpcoming}</p>
            {/if}
            <button onclick={() => setRoute('momen')} class="btn btn-sm btn-ghost mt-2">{tr().dashboard.seeAll}</button>
          </div>
        </div>
      </div>
    </div>
